...
def dequeue_and_enqueue(keys, queue, queue_ptr, queue_size):
    ...
    if torch.cuda.device_count() > 1:
        ...
        gathered_list = [keys]
    keys = torch.cat(gathered_list, dim=0).to(device=keys.device)
...
def label_onehot(inputs, num_segments):
    ...
    outputs = torch.zeros((num_segments, batch_size, im_h, im_w)).to(device=inputs.device)
...
def generate_cutmix_mask(
    ...
    valid_mask = torch.from_numpy(valid_mask).long().to(device=pred.device)
